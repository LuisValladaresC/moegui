...
const gulpNucleus = require('./nunjucks.js');
...
function fileInclude() {
  return gulp
    ...
    .pipe(replace(/@@autopathwithlang/g, function (match) {
      return pathLevel(this.file, paths.data)
    }))
    .pipe(replace(/@@autopath/g, function (match) {
      return pathLevel(this.file)
    }))
    .pipe(gulpNucleus({
      templateRootPath: paths.src.base.dir,
      languagesDataPath: paths.data
    }))
    .pipe(fileinclude({
      prefix: '@@',
      basepath: '@file',
...
function clean() {
  return del([
    paths.build.base.dir,
    paths.src.html.dir + '/es',
    paths.src.html.dir + '/pt'
  ], {force: true});
}

function copyFilesToConfiguredLanguages() {
  return gulp
    .src([
    ...
    .pipe(gulpIf(paths.data['es'] ? true : false, gulp.dest(paths.src.html.dir + '/es')))
    .pipe(gulpIf(paths.data['pt'] ? true : false, gulp.dest(paths.src.html.dir + '/pt')))
}

function inlineCSSForEmailTemplates() {
  return gulp.src(paths.build.base.dir + "/email/**/*.html")
    .pipe(inlineCSS({
      removeStyleTags: false
    }))
    .pipe(gulp.dest(paths.build.base.dir + '/email/'));
}

function sitemapGenerator() {
  return gulp
    .src([
      paths.build.base.dir + '/**/*.html',
      '!' + paths.build.base.dir + '/assets/**/*',
    ])
    .pipe(sitemap({
        siteUrl: 'https://moegui.com',
        noindex: true
    }))
    .pipe(gulp.dest('./build'));
}

gulp.task('build', gulp.series(clean, copyFilesToConfiguredLanguages, fileInclude, buildCSS, buildVendorCSS, buildJS, buildVendorJS, svgCompiler, copeImages, copySkippedFiles, copySkippedNodeFiles, copyDependencies, copyFavicon, inlineCSSForEmailTemplates, sitemapGenerator));
